package com.kh.cafeinme.memberTest.svc;
...
public class MemberSVCImplTest {
	...
	void modifyMember() {
		...
		memberVO.setMember_nickname("테스터1");
		memberVO.setMember_address1("울산광역시 중구 젊음의 5거리");
		memberVO.setMember_address2("32");
	...
	void findID() {
		...
		String MEMBER_ID = memberSVC.findID(MEMBER_NAME, MEMBER_TEL);
		log.info(MEMBER_ID);
	}
	...
	void findPW() {
		...
		String MEMBER_PW = memberSVC.findPW(MEMBER_ID, MEMBER_TEL);
		log.info(MEMBER_PW);
	}
	...
	void existMember() {
		...
		log.info("회원 존재유무:" + memberSVC.existMember(MEMBER_ID));
	}
	...
	void isMember() {
		...
		log.info("회원인증:" + memberSVC.isMember(MEMBER_ID, MEMBER_PW));
	}
}
