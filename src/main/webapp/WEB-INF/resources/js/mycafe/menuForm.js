...
const $menuform = document.querySelector('.menuJoinForm');
const $categorys = document.querySelectorAll(".categoryinfo");
...
function addHandler(evt){
  ...
  $selectBox.setAttribute('name',`menulist[${mcnt}].menu_category`);
  for(let i=0; i<$categorys.length; i++){
    let option = document.createElement("option");
    option.value = $categorys[i].dataset.cno;
    option.textContent = $categorys[i].dataset.cna;
...
function delHandler(evt){
  ...
  for(let i=0; i<x.length; i++){
    if(x[i].parentElement.childNodes.length!=1){
      if(flag){
        confirm = window.confirm("기존메뉴입니다 정말로삭제하시겠습니까")
        flag = false;
      }
      if(confirm){

        del(x[i].parentElement.childNodes[1].value);
      }
      else{
        return;
      }
